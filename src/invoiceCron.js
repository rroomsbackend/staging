...
const generateInvoice = (property, bookings, totalAmount) => {
      ...
      <table border="1" style="width: 100%; border-collapse: collapse;">
        <thead>
          <tr>
            ...
            <th style="padding: 8px; border: 1px solid #ddd;font-size:10px">Total Amount (INCL. Tax)</th>
  ...
  <div style="text-align: center">
    ...
    <p style="padding:0px;margin:0px"><strong>Owner:</strong> ${property.ownerFirstName} ${property.ownerLastName}</p>
  ...
  for (let property of properties) {
    const bookings = await db.BookingHotel.findAll({
      where: {
        ...
        // fromDate: { [Op.gte]: '2025-04-01' },
        // toDate: { [Op.lte]: '2025-04-20' },
...
async function sendInvoiceEmail(property, invoiceDetails, totalAmount, outputPath) {
  ...
  const transporter = nodemailer.createTransport({
    ...
    secure: true,
    auth: {
      user: 'rrooms.in@gmail.com',
      pass: 'dsflwiubogjqkbmh'
    }
  });
  const mailOptions = {
    from: `"RRooms Hospitality" <${process.env.SMTP_FROM}>`,
    to: property.propertyEmailId,
    // to: 'rrooms.in@gmail.com',
    cc: property.ownerEmail,
    bcc: 'rrooms.in@gmail.com',
  ...
  try {
    ...
    console.log('Invoice sent: ' + info.response + ' - ' + property.propertyEmailId);
  } catch (error) {
